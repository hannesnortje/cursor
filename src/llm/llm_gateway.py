...
class CursorLLMProvider:
            ...
            for name, max_tokens, api_base in potential_models:
                models.append(
                    LLMModel(
                        ...
                        provider=LLMProvider.CURSOR,
                        model_type=(
                            ModelType.CODING if "sonnet" in name else ModelType.GENERAL
                        ),
                        max_tokens=max_tokens,
    ...
    async def generate(self, model_name: str, prompt: str, **kwargs) -> str:
        ...
        try:
            model = next(
                (m for m in self.available_models if m.name == model_name), None
            )
            if not model:
        ...
        try:
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                if response.status_code == 200:
                    ...
                    logger.warning(
                        f"Failed to get Ollama models: {response.status_code}"
                    )
        except Exception as e:
            ...
            try:
                async with httpx.AsyncClient(timeout=30.0) as client:
                    ...
                    if response.status_code == 200:
                        ...
                        raise Exception(
                            f"Ollama generation failed: {response.status_code}"
                        )
            except Exception as service_error:
                logger.warning(
                    f"External Docker Ollama service unavailable: {service_error}"
                )
...
class LLMGateway:
    ...
    async def select_best_model(self, task_type: str, context: str = "") -> LLMModel:
        ...
        if task_type == "coding" or "code" in context.lower():
            ...
            for provider_name, provider_models in available_models.items():
                if provider_name in [
                    ...
                    for model in provider_models:
                        if (
                            model.model_type == ModelType.CREATIVE
        ...
        all_models = []
        for provider_name, provider_models in available_models.items():
            if provider_name in ["cursor", "docker_ollama", "lm_studio"] and isinstance(
    ...
    def _select_by_performance(self, models: List[LLMModel]) -> LLMModel:
        ...
        sorted_models = sorted(
            models, key=lambda m: (m.success_rate, -m.response_time), reverse=True
        )
        ...
        try:
            ...
            if preferred_model:
                try:
                    if preferred_model.startswith("cursor-"):
                        return await self.cursor_llms.generate(
                            preferred_model, prompt, **kwargs
                        )
                    else:
                        return await self.docker_ollama.generate(
                            preferred_model, prompt, **kwargs
                        )
                except Exception as e:
            ...
            try:
                if selected_model.provider == LLMProvider.CURSOR:
                    result = await self.cursor_llms.generate(
                        selected_model.name, prompt, **kwargs
                    )
                else:
                    result = await self.docker_ollama.generate(
                        selected_model.name, prompt, **kwargs
                    )
    ...
    async def _try_fallback_generation(
        self, prompt: str, failed_model: LLMModel, **kwargs
    ) -> str:
