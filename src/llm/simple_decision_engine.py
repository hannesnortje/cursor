...
class SimpleDecisionEngine:
    ...
    def __init__(
        ...
        self.logger.info(
            f"  - Local LLM: {'✅ Available' if self.local_llm_available else '❌ Unavailable'}"
        )
        self.logger.info(
            f"  - Cursor LLM: {'✅ Available' if CURSOR_LLM_AVAILABLE else '❌ Unavailable'}"
        )
    ...
    def _rule_based_decision(self, message: str) -> Optional[ProtocolDecision]:
        ...
        has_development_question = any(
            phrase in message_lower
            for phrase in [
                "development approach",
                "what would you recommend for the development",
        ...
        if any(
            ...
            for phrase in [
                "start a new project",
                "business need",
                "not sure exactly what technology",
            ]
        ):
        ...
        has_project_goals = any(
            phrase in message_lower
            for phrase in ["project goals", "main purpose", "project management system"]
        )
        has_objectives = any(
            phrase in message_lower
            for phrase in ["objectives", "target users", "development team"]
        )
        has_tech_requirements = any(
            phrase in message_lower
            for phrase in ["technical requirements", "team expertise", "authentication"]
        )
...
Respond with just the action name:"""
    ...
    def update_conversation_state(
        self, message: str, decision: ProtocolDecision, response: str = None
    ) -> None:
        ...
        if decision.parameters.get("project_details"):
            self.conversation_state.project_details.update(
                decision.parameters["project_details"]
            )
        ...
        self.logger.info(
            f"Conversation state updated - Phase: {self.conversation_state.current_phase.value}"
        )
