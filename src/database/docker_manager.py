...
class QdrantDockerManager:
    ...
    def _start_qdrant_with_subprocess(self) -> bool:
        ...
        try:
            ...
            if self.container_name in result.stdout:
                ...
                subprocess.run(
                    ["docker", "start", self.container_name], check=True, timeout=30
                )
    ...
    def _stop_qdrant_with_subprocess(self) -> bool:
        ...
        try:
            subprocess.run(
                ["docker", "stop", self.container_name], check=True, timeout=30
            )
    ...
    def _remove_qdrant_with_subprocess(self) -> bool:
        ...
        try:
            subprocess.run(
                ["docker", "rm", "-f", self.container_name], check=True, timeout=30
            )
        ...
        while time.time() - start_time < timeout:
            try:
                response = requests.get(
                    f"http://localhost:{self.port}/collections", timeout=5
                )
