...
class QdrantVectorStore:
    ...
    async def store_conversation(self, conversation: ConversationPoint) -> str:
        ...
        try:
            ...
            self.client.upsert(
                collection_name=self.collections["conversations"], points=[point]
            )
    ...
    async def store_project_context(self, project_context: ProjectContext) -> str:
        ...
        try:
            ...
            self.client.upsert(
                collection_name=self.collections["projects"], points=[point]
            )
        ...
        try:
            ...
            if context_type:
                if search_filter:
                    search_filter.must.append(
                        FieldCondition(
                            key="context_type", match=MatchValue(value=context_type)
                        )
                    )
                else:
                    search_filter = Filter(
