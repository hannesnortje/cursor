...
class RateLimiter:
    ...
    def __init__(self):
        ...
        logger.info(f"Rate limiter initialized - default: {self.default_limit}/min, burst: {self.burst_size}")
    ...
    def _load_rate_limits(self) -> Dict[str, Dict[str, int]]:
        ...
        return {
            ...
            "mcp_tools": {
                "requests_per_minute": int(os.getenv("RATE_LIMIT_MCP_TOOLS_PER_MINUTE", "50")),
    ...
    def is_allowed(self, client_id: str, endpoint_type: str = "general") -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            if current_count >= burst_size:
                ...
                recent_requests = [req_time for req_time in window if now - req_time < 10]
                if len(recent_requests) >= burst_size:
                    return {
                        ...
                        "remaining": 0,
                        "reset_time": recent_requests[0] + 10 if recent_requests else now + 10
                    }
    ...
    def reset_client(self, client_id: str, endpoint_type: str = "general") -> bool:
        ...
        with self.lock:
            ...
            if window_key in self.windows:
                ...
                logger.info(f"Rate limit reset for client {client_id} on {endpoint_type}")
                return True
            return False
    ...
    def cleanup_old_windows(self) -> int:
        ...
        with self.lock:
            ...
            if windows_to_remove:
                logger.info(f"Cleaned up {len(windows_to_remove)} old rate limit windows")
