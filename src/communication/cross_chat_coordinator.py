...
class CrossChatService:
    ...
    def stop_service(self) -> Dict[str, Any]:
        ...
        try:
            ...
            uptime = (
                (stop_time - self.start_time).total_seconds() if self.start_time else 0
            )
    ...
    def create_chat_session(
        ...
        try:
            success = self.coordinator.register_chat_session(
                chat_id, chat_type, participants, metadata
            )
    ...
    def unsubscribe_agent(self, agent_id: str, chat_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(
                f"Error unsubscribing agent {agent_id} from chat {chat_id}: {e}"
            )
            return {
                "success": False,
                "error": str(e),
    ...
    def search_messages(
        self, query: str, event_type: Optional[str] = None, limit: int = 50
    ) -> Dict[str, Any]:
