...
class MessageRouter:
    ...
    def __init__(self):
        ...
        self.agent_subscriptions: Dict[str, Set[str]] = {}  # agent_id -> set of chat_ids
    ...
    def create_chat_session(self, session_id: str, chat_type: str, 
                           participants: List[str]) -> ChatSession:
        ...
        for chat_id in message.target_chats:
            if chat_id in self.chat_sessions:
                if self.chat_sessions[chat_id].is_active:
                    ...
                    self.chat_sessions[chat_id].last_activity = datetime.now().isoformat()
    ...
    def broadcast_to_all_chats(self, message: CrossChatMessage) -> Dict[str, Any]:
        ...
        message.target_chats = [
            chat_id for chat_id, session in self.chat_sessions.items()
            if session.is_active
        ]
        
        return self.route_message(message)
    
    def get_chat_messages(self, chat_id: str, limit: int = 100) -> List[CrossChatMessage]:
    ...
    def get_agent_messages(self, agent_id: str, limit: int = 100) -> List[CrossChatMessage]:
    ...
    def search_messages(self, query: str, chat_id: Optional[str] = None, 
                       limit: int = 50) -> List[CrossChatMessage]:
