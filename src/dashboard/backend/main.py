...
app.mount(
    "/static/lib/lit",
    StaticFiles(directory="../frontend/lib/lit/current"),
    name="lit_current",
)
...
async def startup_event():
    ...
    try:
        ...
        if success:
            ...
            logger.info(
                f"✅ Lit 3 ready: {info['size']} bytes, version: {info['version']}"
            )

            # Start background tasks
            await advanced_lit_manager.start_background_tasks()
            logger.info(
                "🚀 Started background tasks for automatic updates and health checks"
            )
        else:
    ...
    try:
        ...
        if instance_id:
            ...
            logger.info(
                f"✅ MCP integration service initialized for instance {instance_id}"
            )
        else:
...
async def download_lit():
    ...
    try:
        ...
        if success:
            ...
            raise HTTPException(
                status_code=500, detail="Failed to download Lit 3 library"
            )
    except Exception as e:
...
async def rollback_to_version(version: str):
    ...
    try:
        ...
        if success:
            ...
            raise HTTPException(
                status_code=400, detail=f"Failed to rollback to version {version}"
            )
    except Exception as e:
...
async def get_lit_health():
    ...
    try:
        ...
        health_status = {
            ...
            "cdn_sources_healthy": sum(
                1 for source in info.get("cdn_sources", []) if source["healthy"]
            ),
...
async def shutdown_dashboard(request: ShutdownRequest):
    ...
    try:
        logger.info(
            f"🔄 Dashboard shutdown requested: {request.reason} at {request.timestamp}"
        )
        ...
        if instance_id:
            try:
                ...
                if registry:
                    ...
                    logger.info(
                        f"✅ Instance {instance_id} marked as stopped in registry"
                    )
                else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--port", type=int, default=5000, help="Port to run the dashboard on"
    )
    parser.add_argument(
        "--instance-id", type=str, help="MCP instance ID this dashboard belongs to"
    )
    ...
    logger.info(
        f"🚀 Starting Dashboard Backend for instance {instance_id} on port {port}..."
    )
