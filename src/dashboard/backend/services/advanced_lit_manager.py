...
class AdvancedLitManager:
            ...
            try:
                with open(self.metrics_file, "r") as f:
                    ...
                    if data.get("last_update"):
                        data["last_update"] = datetime.fromisoformat(
                            data["last_update"]
                        )
                    if data.get("uptime_start"):
                        data["uptime_start"] = datetime.fromisoformat(
                            data["uptime_start"]
                        )
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.head(
                    source["url"], timeout=aiohttp.ClientTimeout(total=10)
                ) as response:
    ...
    async def _get_healthy_sources(self) -> List[Dict[str, Any]]:
        ...
        for source in self.cdn_sources:
            ...
            if (
                not source["last_check"]
                or (datetime.now() - source["last_check"]).total_seconds()
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    ...
                    version_info = VersionInfo(
                        ...
                        / (download_time + 0.1),  # Higher score for faster downloads
                    )

                    # Save to versions directory (sanitize version for filesystem)
                    safe_version = (
                        version_info.version.replace('"', "")
                        .replace("/", "_")
                        .replace("\\", "_")
                    )
                ...
                async with session.head(
                    ...
                    if response.status == 200:
                        ...
                        if latest_etag and latest_etag != self.current_version.version:
                            logger.info(
                                f"Update available: {self.current_version.version} -> {latest_etag}"
                            )
                            return True
    ...
    async def ensure_lit_available(self) -> bool:
        ...
        try:
            ...
            if not self.current_version:
                ...
                success, version_info = await self._download_from_source(
                    healthy_sources[0]
                )
                if success and version_info:
                    await self._activate_version(version_info)
            ...
            if version_dir.is_dir():
                ...
                if version_file.exists():
                    try:
                        ...
                        logger.warning(
                            f"Failed to read version info for {version_dir.name}: {e}"
                        )
