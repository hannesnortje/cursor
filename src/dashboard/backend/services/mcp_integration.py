...
class MCPIntegrationService:
    ...
    def __init__(self, instance_id: str = None):
        ...
        self.mcp_server_path = Path("/media/hannesn/storage/Code/cursor/protocol_server.py")
        ...
        try:
            ...
            if self.registry and self.instance_id:
                ...
                if instance_info and instance_info.status == "running":
                    ...
                    logger.info(f"MCP server connection verified for instance {self.instance_id}")
                    return
            ...
            for proc in psutil.process_iter(['pid', 'name', 'cmdline']):
                try:
                    if proc.info['cmdline'] and 'protocol_server.py' in ' '.join(proc.info['cmdline']):
                        # Create a dummy process object to track the found MCP server
                        self.mcp_process = type('Process', (), {'pid': proc.info['pid'], 'poll': lambda self=None: None})()
                        self.is_connected = True
                        logger.info(f"Found running MCP server process: {proc.info['pid']}")
                        return
                ...
                return [
                    {
                        ...
                        "uptime": uptime_str,
                        "process_id": mcp_pid,
                        "memory_usage": round(process.memory_info().rss / 1024 / 1024, 1) if 'process' in locals() else 0,
                        "cpu_usage": round(process.cpu_percent(), 1) if 'process' in locals() else 0
                    }
                ]
                        ...
                        return [
                            {
                                ...
                                "uptime": self._calculate_uptime(instance_info.started_at),
