...
// Wait for Lit to be loaded, then define the componentngetLit().then(async ({ LitElement, html, css }) => {nnexport class QuickActionsCard extends LitElement {
  static properties = {};
...
console.log('✅ Quick Actions Card component registered successfully');

}).catch(error => {
  console.error('❌ Failed to load Lit 3 for Quick Actions Card:', error);
  
  // Create a fallback component that shows the error
  class QuickActionsCardError extends HTMLElement {
    connectedCallback() {
      this.innerHTML = `
        <div style="
          padding: 1rem;
          ...
          text-align: center;
        ">
          <div style="font-weight: bold; margin-bottom: 0.5rem;">❌ Quick Actions Card Failed</div>
  ...
  customElements.define('quick-actions-card', QuickActionsCardError);
});
