...
// Wait for Lit to be loaded, then define the componentngetLit().then(async ({ LitElement, html, css }) => {nnexport class PerformanceCard extends LitElement {
  static properties = {
...
console.log('✅ Performance Card component registered successfully');

}).catch(error => {
  console.error('❌ Failed to load Lit 3 for Performance Card:', error);
  
  // Create a fallback component that shows the error
  class PerformanceCardError extends HTMLElement {
    connectedCallback() {
      this.innerHTML = `
        <div style="
          padding: 1rem;
          ...
          text-align: center;
        ">
          <div style="font-weight: bold; margin-bottom: 0.5rem;">❌ Performance Card Failed</div>
  ...
  customElements.define('performance-card', PerformanceCardError);
});
