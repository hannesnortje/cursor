...
class BrowserManager:
    ...
    def _is_browser_available(self, browser_info: BrowserInfo) -> bool:
        ...
        try:
            ...
            if (
                browser_info.browser_type == BrowserType.SAFARI
                and self.system == "darwin"
            ):
                result = subprocess.run(
                    ["which", "open"], capture_output=True, text=True, timeout=5
                )
            ...
            result = subprocess.run(
                (
                    ["which", browser_info.command]
                    if self.system != "windows"
                    else ["where", browser_info.command]
                ),
                capture_output=True,
                text=True,
                timeout=5,
    ...
    def open_dashboard(
        ...
        try:
            ...
            if instance_id:
                logger.info(
                    f"Dashboard for instance {instance_id} opening in {browser.name}"
                )
    ...
    def get_browser_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "preferred_browser": (
                self.preferred_browser.name if self.preferred_browser else None
            ),
    ...
    def test_browser_opening(
        self, test_url: str = "https://www.google.com"
    ) -> Dict[str, Any]:
        ...
        for browser in self.detected_browsers:
            try:
                ...
                success = self.open_dashboard(
                    test_url, browser_type=browser.browser_type
                )
...
def open_dashboard_in_browser(
    url: str, instance_id: str = None, browser_type: Optional[BrowserType] = None
) -> bool:
