...
class AgileAgent(BaseAgent):
    ...
    def __init__(self, agent_id: str = None, name: str = "Agile Agent"):
        ...
        self.velocity_calculation_sprints = 3  # Number of sprints for velocity calculation
        
        logger.info(f"Agile Agent {self.agent_id} initialized with capabilities: {[cap.name for cap in self.capabilities]}")
    ...
    async def _handle_create_project_task(self, task) -> Dict[str, Any]:
        ...
        return self.create_agile_project(project_name, project_type, sprint_length, team_size)
    ...
    async def _handle_create_user_story_task(self, task) -> Dict[str, Any]:
        ...
        return self.create_user_story(project_id, title, description, 
    ...
    def create_user_story(self, project_id: str, title: str, description: str,
        ...
        try:
            ...
            if story_points is None:
                story_points = self._auto_estimate_story_points(description, acceptance_criteria)
            ...
            logger.info(f"Created user story: {title} ({story_id}) with {story_points} points")
    ...
    def _auto_estimate_story_points(self, description: str, acceptance_criteria: List[str]) -> int:
        ...
        complex_keywords = ["integration", "api", "database", "security", "performance", "testing"]
    ...
    def create_sprint(self, project_id: str, sprint_name: str, start_date: str = None,
        ...
        try:
            ...
            logger.info(f"Created sprint: {sprint_name} ({sprint_id}) for project {project_id}")
        ...
        try:
            ...
            for story_id in story_ids:
                if story_id in self.user_stories:
                    ...
                    if story.status == "backlog" and story.project_id == sprint.project_id:
            ...
            logger.info(f"Planned sprint {sprint_id} with {len(valid_stories)} stories ({total_points} points)")
    ...
    def calculate_team_velocity(self, project_id: str, sprint_count: int = None) -> Dict[str, Any]:
        ...
        try:
            ...
            completed_sprints = [
                sprint for sprint in self.active_sprints.values()
            ...
            # Calculate velocity
            total_points = sum(sprint.completed_story_points for sprint in recent_sprints)
            ...
            if len(recent_sprints) >= 2:
                ...
                velocity_trend = "increasing" if recent_velocity > previous_velocity else "decreasing" if recent_velocity < previous_velocity else "stable"
            else:
                velocity_trend = "insufficient_data"
            
            logger.info(f"Calculated team velocity for project {project_id}: {average_velocity:.1f} points/sprint")
            ...
            return {
                ...
                "total_points": total_points,
                "velocity_range": {
                    "min": min(sprint.completed_story_points for sprint in recent_sprints),
                    "max": max(sprint.completed_story_points for sprint in recent_sprints)
                }
            }
        ...
        try:
            ...
            for day in range(sprint_days + 1):
                ...
                burndown_data.append({
                    ...
                    "actual_points": remaining_points if day == sprint_days else None,
                    "remaining_points": remaining_points if day == sprint_days else None
                })
            ...
            return {
                ...
                "completion_percentage": round((sprint.completed_story_points / total_points) * 100, 1) if total_points > 0 else 0
            }
    ...
    def get_project_status(self, project_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            completed_stories = len([s for s in project_stories if s.status == "completed"])
            in_progress_stories = len([s for s in project_stories if s.status == "in_progress"])
            ...
            return {
                ...
                "metrics": {
                    ...
                    "points_completion_percentage": round((project.completed_story_points / project.total_story_points) * 100, 1) if project.total_story_points > 0 else 0
                },
                "sprints": {
                    "total": len(project_sprints),
                    "active": len([s for s in project_sprints if s.status == "active"]),
                    "completed": len([s for s in project_sprints if s.status == "completed"]),
                    "planning": len([s for s in project_sprints if s.status == "planning"])
                },
