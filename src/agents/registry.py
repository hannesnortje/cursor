...
class AgentRegistry:
    ...
    async def register_agent(self, agent: BaseAgent) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"Registered agent: {agent.name} ({agent.agent_type.value})"
            )
            return True
            ...
            if not target_agent:
                ...
                for agent in self.agents.values():
                    if (
                        ...
                        and len(agent.tasks) < agent.max_concurrent_tasks
                    ):
                        target_agent = agent
                        break

            if not target_agent:
                return {"success": False, "error": "No available agents to handle task"}
    ...
    async def get_registry_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "registry_uptime": (
                datetime.now() - self.registry_start_time
            ).total_seconds(),
                ...
                for agent_id, agent in self.agents.items():
                    try:
                        ...
                        if agent_id in self.agent_info:
                            ...
                            self.agent_info[agent_id].last_activity = (
                                agent.last_activity
                            )
