...
class CoordinatorAgent(BaseAgent):
    ...
    def __init__(
        self, agent_id: str = "coordinator_001", name: str = "System Coordinator"
    ):
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            self.register_message_handler(
                "start_pdca_cycle", self._handle_start_pdca_cycle
            )
            ...
            self.register_message_handler(
                "project_generation", self._handle_project_generation_request
            )
...
- Set up the testing framework with the Testing Agent?""",
    ...
    async def assign_task_to_agent(
        ...
        try:
            ...
            result = await self.agent_registry.assign_task_to_agent(
                task_data, preferred_agent_type=preferred_agent_type
            )
    ...
    async def advance_project_phase(self, project_id: str) -> bool:
        ...
        try:
            ...
            if success:
                ...
                self.logger.info(
                    f"Advanced project {project.name} to phase: {project.metadata.get('current_pdca_phase')}"
                )
    ...
    async def get_system_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            completed_projects = len(
                [p for p in self.project_plans.values() if p.status == "completed"]
            )
    ...
    async def _handle_start_pdca_cycle(
        self, message_data: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def create_project_from_template(
        ...
        try:
            self.logger.info(
                f"Coordinator: Creating project '{project_name}' using template '{template_id}'"
            )
            ...
            if not project_gen_agent:
                return {
                    "success": False,
                    "error": "Failed to create Project Generation Agent",
                }
            ...
            if result["success"]:
                self.logger.info(
                    f"Coordinator: Project '{project_name}' created successfully"
                )
            ...
            self.logger.error(
                f"Coordinator: Failed to create project from template: {e}"
            )
            return {"success": False, "error": str(e)}
    ...
    async def create_custom_project(
        ...
        try:
            self.logger.info(
                f"Coordinator: Creating custom project '{project_name}' in {language}"
            )
            ...
            if result["success"]:
                self.logger.info(
                    f"Coordinator: Custom project '{project_name}' created successfully"
                )
                # Create a project plan for the generated project
                await self._create_project_plan_for_generated_project(result)
    ...
    async def list_project_templates(
        ...
        try:
            self.logger.info(
                f"Coordinator: Listing project templates (language: {language}, category: {category})"
            )
            ...
            self.logger.info(
                f"Coordinator: Retrieved {result.get('total_count', 0)} project templates"
            )
            return result
    ...
    async def customize_project_template(
        ...
        try:
            ...
            if result["success"]:
                self.logger.info(
                    f"Coordinator: Template '{template_id}' customized successfully"
                )
    ...
    async def get_generated_project_status(self, project_id: str) -> Dict[str, Any]:
        ...
        try:
            self.logger.info(
                f"Coordinator: Getting status for generated project '{project_id}'"
            )
            ...
            self.logger.error(
                f"Coordinator: Failed to get generated project status: {e}"
            )
            return {"success": False, "error": str(e)}
    ...
    async def list_generated_projects(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(
                f"Coordinator: Retrieved {result.get('total_count', 0)} generated projects"
            )
            return result
    ...
    async def _get_or_create_project_generation_agent(self):
        ...
        try:
            ...
            if agent_result.get("success"):
                ...
                self.logger.error(
                    f"Coordinator: Failed to create Project Generation Agent: {agent_result.get('error')}"
                )
                return None

        except Exception as e:
            self.logger.error(
                f"Coordinator: Error getting/creating Project Generation Agent: {e}"
            )
            return None

    async def _create_project_plan_for_generated_project(
        ...
        try:
            ...
            if project_plan:
                self.logger.info(
                    f"Coordinator: Created project plan for generated project '{project_name}'"
                )
            ...
            self.logger.error(
                f"Coordinator: Failed to create project plan for generated project: {e}"
            )

    async def _handle_project_generation_request(
        self, message_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if request_type == "create_from_template":
                ...
                return {
                    "success": False,
                    "error": f"Unknown project generation request type: {request_type}",
                }

        except Exception as e:
            self.logger.error(
                f"Coordinator: Failed to handle project generation request: {e}"
            )
            return {"success": False, "error": str(e)}
