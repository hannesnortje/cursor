...
class PDCAFramework:
    ...
    async def advance_phase(self, cycle_id: str) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"Advanced cycle {cycle.name} to phase: {cycle.current_phase.value}"
            )
            return True
    ...
    async def update_objective_progress(
        self, objective_id: str, completion_percentage: float, status: str = None
    ) -> bool:
        ...
        try:
            ...
            self.logger.debug(
                f"Updated objective {objective_id} progress: {completion_percentage}%"
            )
            return True
    ...
    async def list_cycles(
        self, status_filter: Optional[PDCAStatus] = None
    ) -> List[Dict[str, Any]]:
    ...
    def get_framework_status(self) -> Dict[str, Any]:
        ...
        active_cycles = len(
            [c for c in self.cycles.values() if c.status == PDCAStatus.IN_PROGRESS]
        )
        completed_cycles = len(
            [c for c in self.cycles.values() if c.status == PDCAStatus.COMPLETED]
        )
