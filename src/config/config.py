...
class AppConfig:
    ...
    def _load_from_env(self):
        ...
        if os.getenv("ENABLE_COORDINATOR"):
            self.agent.enable_coordinator = (
                os.getenv("ENABLE_COORDINATOR").lower() == "true"
            )
        ...
        if os.getenv("ENABLE_VECTOR_DB"):
            self.database.enable_vector_db = (
                os.getenv("ENABLE_VECTOR_DB").lower() == "true"
            )

        # LLM Configuration
        if os.getenv("ENABLE_CURSOR_LLM"):
            self.llm.enable_cursor_llm = (
                os.getenv("ENABLE_CURSOR_LLM").lower() == "true"
            )
        if os.getenv("ENABLE_DOCKER_OLLAMA"):
            self.llm.enable_docker_ollama = (
                os.getenv("ENABLE_DOCKER_OLLAMA").lower() == "true"
            )
