...
TEST_STORY_DESCRIPTION = "Implement secure user authentication system with login/logout functionality"
    ...
    if response and "result" in response:
        ...
        if "structuredContent" in result:
            ...
            if project_data.get("success"):
                ...
                log_test(f"❌ Failed to create agile project: {project_data.get('error')}", "FAIL")
                return None
        else:
    ...
    if response and "result" in response:
        ...
        if "structuredContent" in result:
            ...
            if story_data.get("success"):
                ...
                log_test(f"✅ User story created successfully: {story_id} ({story_points} points)")
                return story_id
            else:
                log_test(f"❌ Failed to create user story: {story_data.get('error')}", "FAIL")
                return None
        else:
            log_test("❌ No structured content in response", "FAIL")
            return None
    ...
    if response and "result" in response:
        ...
        if "structuredContent" in result:
            ...
            if sprint_data.get("success"):
                ...
                log_test(f"❌ Failed to create sprint: {sprint_data.get('error')}", "FAIL")
                return None
        else:
            log_test("❌ No structured content in response", "FAIL")
            return None
    ...
    if response and "result" in response:
        ...
        if "structuredContent" in result:
            ...
            if plan_data.get("success"):
                ...
                log_test(f"✅ Sprint planned successfully: {planned_stories} stories ({total_points} points)")
                return True
            else:
    ...
    if response and "result" in response:
        ...
        if "structuredContent" in result:
            ...
            if status_data.get("success"):
                ...
                log_test(f"✅ Project status retrieved: {total_stories} stories, {total_points} points")
                return True
            else:
                log_test(f"❌ Failed to get project status: {status_data.get('error')}", "FAIL")
                return False
        else:
            log_test("❌ No structured content in response", "FAIL")
            return False
    ...
    if response and "result" in response:
        ...
        if "structuredContent" in result:
            ...
            if burndown_data.get("success"):
                ...
                log_test(f"✅ Sprint burndown generated: {total_points} points, {completion_percentage}% complete")
                return True
            else:
                log_test(f"❌ Failed to generate burndown: {burndown_data.get('error')}", "FAIL")
                return False
        else:
            log_test("❌ No structured content in response", "FAIL")
            return False
    ...
    if response and "result" in response:
        ...
        if "structuredContent" in result:
            ...
            if velocity_data.get("success"):
                log_test("✅ Team velocity calculated (expected: no completed sprints yet)")
                return True
            else:
                ...
                if "No completed sprints found" in error_msg:
                    log_test("✅ Team velocity calculation handled correctly (no completed sprints)")
                    return True
                else:
                    log_test(f"❌ Unexpected error in velocity calculation: {error_msg}", "FAIL")
                    return False
        else:
            log_test("❌ No structured content in response", "FAIL")
            return False
    ...
    if response and "result" in response:
        ...
        if "structuredContent" in result:
            ...
            if completion_data.get("success"):
                ...
                log_test(f"✅ User story completed successfully: {completed_points} points")
                return True
            else:
                log_test(f"❌ Failed to complete user story: {completion_data.get('error')}", "FAIL")
                return False
        else:
            log_test("❌ No structured content in response", "FAIL")
            return False
