...
def test_network_monitoring():
    ...
    try:
        ...
        print(
            f"✅ Network Statistics: {stats['total_connections']} connections, monitoring: {stats['monitoring_active']}"
        )
        ...
        stats = network_monitor.get_network_statistics()
        print(
            f"✅ Monitoring Started: {stats['monitoring_active']}, interval: {stats['monitor_interval']}s"
        )
...
def test_ssl_configuration():
    ...
    try:
        ...
        print(
            f"✅ SSL Validation: {validation['valid']}, {validation['contexts']} contexts, {validation['certificates']} certificates"
        )
        ...
        try:
            result = ssl_config.test_ssl_connection(
                "www.google.com", 443, "test_context"
            )
            if result["success"]:
                print(
                    f"✅ SSL Connection Test: Connected to {result['host']}:{result['port']}"
                )
            else:
        ...
        try:
            ...
            if result["success"]:
                print(
                    f"✅ Plain Connection Test: Connected to {result['host']}:{result['port']}"
                )
            else:
...
def test_firewall_management():
    ...
    try:
        ...
        print(
            f"✅ Firewall Status: {status['total_rules']} rules, {len(status['blocked_ips'])} blocked IPs"
        )
        ...
        print(
            f"✅ IP Block Check: {test_ip} is {'blocked' if is_blocked else 'allowed'}"
        )
        ...
        print(
            f"✅ IP Allow Check: {test_ip} is {'allowed' if is_allowed else 'blocked'}"
        )
        ...
        print(
            f"✅ Port Block Check: {test_port} is {'blocked' if is_blocked else 'allowed'}"
        )

        # Test 8: Get updated status
        status = firewall_manager.get_firewall_status()
        print(
            f"✅ Updated Firewall Status: {status['total_rules']} rules, {len(status['blocked_ips'])} blocked IPs, {len(status['blocked_ports'])} blocked ports"
        )
...
def test_network_connectivity():
    ...
    try:
        ...
        result = firewall_manager.test_network_connectivity(
            "nonexistent.example.com", 80, 2
        )
...
def test_mcp_tools():
    ...
    try:
        ...
        handle_system_tool(
            "get_network_connections", {"limit": 10}, "test_2", mock_send_response
        )
        if responses and responses[-1]["result"]:
...
def run_phase_4_1_test():
    ...
    if passed == total:
        print(
            "🎉 ALL PHASE 4.1 TESTS PASSED! Network Security Configuration is working perfectly!"
        )
        return True
    else:
