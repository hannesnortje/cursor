...
class DashboardTester:
        ...
        try:
            ...
            try:
                response = requests.get(
                    f"{self.base_url}/static/lib/lit/current/lit-core.min.js", timeout=5
                )
                results["static_files"] = response.status_code == 200
                print(
                    f"✅ Static files: {'Working' if results['static_files'] else 'Failed'}"
                )
        ...
        for endpoint, name in endpoints.items():
            try:
                ...
                results[endpoint] = {
                    ...
                    in [200, 404],  # 404 is OK for some endpoints
                    "response_time": response.elapsed.total_seconds(),
                }

                status = "✅" if results[endpoint]["accessible"] else "❌"
                print(
                    f"  {status} {name}: {response.status_code} ({results[endpoint]['response_time']:.2f}s)"
                )
    ...
    def test_performance(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                print(
                    f"✅ Concurrent requests: {'Working' if results['concurrent_requests'] else 'Failed'}"
                )
    ...
    def test_user_experience(self) -> Dict[str, Any]:
        ...
        try:
            ...
            print(
                f"✅ Responsive design: {'Present' if results['responsive_design'] else 'Missing'}"
            )
            ...
            print(
                f"✅ Error handling: {'Working' if results['error_handling'] else 'Needs improvement'}"
            )

            # Test navigation elements
            navigation_elements = ["nav", "header", "footer", "dashboard"]
            results["navigation"] = any(
                element in html_content.lower() for element in navigation_elements
            )
    ...
    def generate_summary(self):
        ...
        basic_success = sum(
            1 for v in self.results["basic_functionality"].values() if v is True
        )
        basic_total = len(
            [k for k in self.results["basic_functionality"].keys() if k != "load_time"]
        )

        api_success = sum(
            1 for ep in self.results["api_endpoints"].values() if ep["accessible"]
        )
        api_total = len(self.results["api_endpoints"])

        ux_success = sum(
            1 for v in self.results["user_experience"].values() if v is True
        )
        ...
        perf_score = 0
        if (
            self.results["performance"]["load_time"]
            and self.results["performance"]["load_time"] < 5.0
        ):
            perf_score += 1
