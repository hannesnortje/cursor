...
async def test_llm_gateway():
    ...
    try:
        ...
        best_model = await llm_gateway.select_best_model(
            "coding", "Write a Python function"
        )
        print(
            f"    ✅ Best model for coding: {best_model.name} ({best_model.provider.value})"
        )
...
async def test_autogen_integration():
    ...
    print(
        "  ⚠️  This test is deprecated. AutoGen functionality has been moved to enhanced_autogen."
    )
...
async def test_enhanced_communication():
    ...
    try:
        ...
        print(
            f"    ✅ Created session: {session['id']} with {len(session['agent_ids'])} agents"
        )
        ...
        visible_sessions = await enhanced_communication.get_cross_chat_visibility(
            "agent1"
        )
        ...
        success = await enhanced_communication.add_agent_to_session(
            "test_session", "agent3"
        )
