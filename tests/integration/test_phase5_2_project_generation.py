...
def test_phase5_2_project_generation():
    ...
    print("\n📋 **Test 1: List All Project Templates**")
    print("-" * 40)
    response = send_request("tools/call", {"name": "list_project_templates"})
    ...
    if response and "result" in response:
        ...
        if "structuredContent" in result and result["structuredContent"]["success"]:
            ...
            for i, template in enumerate(templates[:5]):
                print(
                    f"  {i+1}. {template['name']} ({template['language']}/{template['category']})"
                )
                print(f"     ID: {template['template_id']}")
                print(
                    f"     Build: {template['build_system']}, Testing: {template['testing_framework']}"
                )
        else:
            print(
                f"❌ Failed to list templates: {result.get('error', 'Unknown error')}"
            )
    else:
    ...
    response = send_request(
        "tools/call",
        {"name": "list_project_templates", "arguments": {"language": "python"}},
    )
    ...
    if response and "result" in response:
        ...
        if "structuredContent" in result and result["structuredContent"]["success"]:
            ...
            for template in templates:
                ...
                print(
                    f"     Framework: {template['framework']}, Category: {template['category']}"
                )
        else:
            print(
                f"❌ Failed to list Python templates: {result.get('error', 'Unknown error')}"
            )
    else:
        print("❌ No response from server")
    ...
    response = send_request(
        "tools/call",
        {"name": "list_project_templates", "arguments": {"category": "api"}},
    )
    ...
    if response and "result" in response:
        ...
        if "structuredContent" in result and result["structuredContent"]["success"]:
            ...
            for template in templates:
                ...
                print(
                    f"     Framework: {template['framework']}, Build: {template['build_system']}"
                )
        else:
            print(
                f"❌ Failed to list API templates: {result.get('error', 'Unknown error')}"
            )
    else:
        print("❌ No response from server")
    ...
    if response and "result" in response:
        ...
        if "structuredContent" in result and result["structuredContent"]["success"]:
            ...
            print(
                f"📦 Dependencies: {', '.join(project_data['project_structure']['dependencies'])}"
            )
            ...
            print(
                f"❌ Failed to generate Flask project: {result.get('error', 'Unknown error')}"
            )
    ...
    if response and "result" in response:
        ...
        if "structuredContent" in result and result["structuredContent"]["success"]:
            project_data = result["structuredContent"]
            print(
                f"✅ Successfully generated C++ project: {project_data['project_name']}"
            )
            print(f"📁 Project ID: {project_data['project_id']}")
            print(f"🌍 Language: {project_data['language']}")
            print(f"⚙️ Framework: {project_data['framework']}")
            ...
            print(
                f"❌ Failed to generate C++ project: {result.get('error', 'Unknown error')}"
            )
            cpp_project_id = None
    else:
        print("❌ No response from server")
    ...
    if response and "result" in response:
        ...
        if "structuredContent" in result and result["structuredContent"]["success"]:
            project_data = result["structuredContent"]
            print(
                f"✅ Successfully generated TypeScript project: {project_data['project_name']}"
            )
            print(f"📁 Project ID: {project_data['project_id']}")
            print(f"🌍 Language: {project_data['language']}")
            print(f"⚙️ Framework: {project_data['framework']}")
            ...
            print(
                f"❌ Failed to generate TypeScript project: {result.get('error', 'Unknown error')}"
            )
            ts_project_id = None
    else:
        print("❌ No response from server")
    ...
    print("\n📋 **Test 7: List All Generated Projects**")
    print("-" * 40)
    response = send_request("tools/call", {"name": "list_generated_projects"})
    ...
    if response and "result" in response:
        ...
        if "structuredContent" in result and result["structuredContent"]["success"]:
            ...
            print(
                f"❌ Failed to list generated projects: {result.get('error', 'Unknown error')}"
            )
    else:
        print("❌ No response from server")
    ...
    if flask_project_id:
        ...
        response = send_request(
            "tools/call",
            {
                ...
                "arguments": {"project_id": flask_project_id},
            },
        )
        ...
        if response and "result" in response:
            ...
            if "structuredContent" in result and result["structuredContent"]["success"]:
                ...
                print(
                    f"✅ Successfully retrieved status for: {project['project_name']}"
                )
                ...
                print(
                    f"❌ Failed to get Flask project status: {result.get('error', 'Unknown error')}"
                )
        else:
            print("❌ No response from server")
    ...
    if response and "result" in response:
        ...
        if "structuredContent" in result and result["structuredContent"]["success"]:
            ...
            print(
                f"📦 Enhanced dependencies: {', '.join(custom_data['new_template']['dependencies'])}"
            )
            print(f"🔧 Customizations applied: {custom_data['customizations_applied']}")
        else:
            print(
                f"❌ Failed to customize template: {result.get('error', 'Unknown error')}"
            )
    else:
        print("❌ No response from server")
    ...
    for lang in languages_to_test:
        ...
        response = send_request(
            "tools/call",
            {"name": "list_project_templates", "arguments": {"language": lang}},
        )
        ...
        if response and "result" in response:
            ...
            if "structuredContent" in result and result["structuredContent"]["success"]:
                ...
                print(
                    f"  ❌ Failed to list {lang} templates: {result.get('error', 'Unknown error')}"
                )
        else:
