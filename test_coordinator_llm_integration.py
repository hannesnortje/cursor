...
I'd specifically like to use Vue 3 with TypeScript for the frontend. What technology stack would you recommend?"""
    ...
    print(f"  - Current Phase: {coordinator.decision_engine.get_conversation_state().current_phase.value}")
    print(f"  - Technology Stack: {coordinator.decision_engine.get_conversation_state().technology_stack}")
    print(f"  - Created Agents: {coordinator.decision_engine.get_conversation_state().created_agents}")
    print(f"  - Conversation History: {len(coordinator.decision_engine.get_conversation_state().conversation_history)} messages")
