...
try:
    ...
    from src.mcp_tools.consolidated_handlers import ConsolidatedToolHandlers
    ...
    for line in sys.stdin:
        try:
            ...
            if method == "initialize":
                ...
                # Import consolidated tools configuration
                from src.mcp_tools.tools_config_consolidated import get_tools_config
                tools_response = get_tools_config()
                send_response(request_id, tools_response)
            elif method == "tools/call":
                # Handle tool calls using consolidated handlers
                tool_name = data.get("params", {}).get("name")
                arguments = data.get("params", {}).get("arguments", {})
                
                try:
                    # Initialize consolidated tool handlers
                    tool_handlers = ConsolidatedToolHandlers(agent_system)
                    result = tool_handlers.handle_tool_call(tool_name, arguments)
                    ...
                    if "error" in result:
                        ...
                        send_response(request_id, result=result.get("result", result))
