    ...
    try:
        ...
        for i, scenario in enumerate(test_scenarios, 1):
            ...
            if response.get("success"):
                print(
                    f"📤 Response: {response.get('response', 'No response')[:200]}..."
                )
                ...
                if response.get("agent_details"):
                    print(
                        f"📋 Agent Details: {len(response.get('agent_details', []))} agents"
                    )
                ...
                if response.get("phase") == scenario["expected_phase"]:
                    ...
                    print(
                        f"⚠️ Phase doesn't match expected: {scenario['expected_phase']}"
                    )
            else:
                print(
                    f"❌ Failed to process message: {response.get('error', 'Unknown error')}"
                )
